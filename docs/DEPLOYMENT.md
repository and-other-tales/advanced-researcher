...
We recommend when deploying Advanced Researcher, you use Vercel for the frontend, GCP Cloud Run for the backend API, and GitHub action for the recurring ingestion tasks. This setup provides a simple and effective way to deploy and manage your application.

## Prerequisites

First, fork [advanced-researcher](https://github.com/and-other-tales/advanced-researcher) to your GitHub account.
...
On the next screen, search for "advanced-researcher" (if you did not modify the repo name when forking). Once shown, click "Import".
...
```shell
gcloud run deploy advanced-researcher --source . --port 8000 --env-vars-file .env.gcp.yaml --allow-unauthenticated --region europe-west4 --min-instances 1
```
